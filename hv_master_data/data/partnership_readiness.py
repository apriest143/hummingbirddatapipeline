...
across four axes:
  ...
  Axis 1: STATE POLICY ENVIRONMENT (28 pts)
    - Outdoor economy % of state GDP (BEA 2024)         0-12 pts
    - Enrollment cliff severity (WICHE 2024 projections) 0-10 pts
    - Prior closure activity in state 2020-2025          0-6 pts

  Axis 2: FUNDING ENVIRONMENT (24 pts)
    - Rural development program access                    0-8 pts
    - State higher ed funding trend                       0-8 pts
    - Accreditation body flexibility                      0-8 pts

  Axis 3: GOVERNANCE & COMMUNITY URGENCY (28 pts)
    - Institution distress trajectory                     0-12 pts
    - Urbanization / community receptiveness              0-10 pts
    - Institutional distress flags                        0-6 pts

  Axis 4: ASSET VALUE POTENTIAL (20 pts)             [NEW v2]
    - Real estate market strength (state/metro)          0-8 pts
    - Acreage quantum (meaningful land holdings)         0-7 pts
    - Land type value modifier                           0-5 pts
    NOTE: Does NOT penalize rural or urban. Large rural acreage
    in appreciating markets scores equally well as urban parcels
    in high-value metros.
...
Sources:
  ...
  Zillow/CoStar state appreciation proxies (2022-2025)
...
# ---------------------------------------------------------------------------
# STATE REAL ESTATE APPRECIATION TIER (2022-2025 trend)
# Proxy for land value trajectory — higher tier = appreciating market
# Sources: Zillow State-Level HVI, CoStar commercial land index, FHFA HPI
# Tier 3 = strong appreciation (>8% annualized)
# Tier 2 = moderate appreciation (3-8% annualized)
# Tier 1 = flat or declining (<3% annualized)
# ---------------------------------------------------------------------------
STATE_RE_APPRECIATION = {
    # Strong appreciation
    'Florida':        3, 'Texas':          3, 'North Carolina':  3,
    'South Carolina': 3, 'Tennessee':      3, 'Georgia':         3,
    'Arizona':        3, 'Nevada':         3, 'Montana':         3,
    'Idaho':          3, 'Utah':           3, 'Colorado':        3,
    'Delaware':       3, 'Vermont':        3, 'Maine':           3,
    'New Hampshire':  3, 'New Jersey':     3, 'Connecticut':     3,
    # Moderate appreciation
    'Virginia':       2, 'Washington':     2, 'Oregon':          2,
    'California':     2, 'Massachusetts':  2, 'Maryland':        2,
    'New York':       2, 'Pennsylvania':   2, 'Ohio':            2,
    'Indiana':        2, 'Michigan':       2, 'Wisconsin':       2,
    'Minnesota':      2, 'Missouri':       2, 'Kansas':          2,
    'Nebraska':       2, 'Iowa':           2, 'Kentucky':        2,
    'Alabama':        2, 'Arkansas':       2, 'Oklahoma':        2,
    'New Mexico':     2, 'Wyoming':        2, 'South Dakota':    2,
    'North Dakota':   2, 'Alaska':         2, 'Hawaii':          2,
    'Rhode Island':   2,
    # Flat / declining
    'Illinois':       1, 'Louisiana':      1, 'Mississippi':     1,
    'West Virginia':  1, 'District of Columbia': 1,
}

# ---------------------------------------------------------------------------
# URBAN LAND VALUE MODIFIER
# Urban core institutions own scarce, developable parcels worth more per acre.
# Rural institutions have large acreage with development potential at scale.
# Both are rewarded — just differently. Neither is penalized.
# ---------------------------------------------------------------------------
URBAN_LAND_MODIFIER = {
    'City':   5,   # High per-acre value, redevelopment potential
    'Suburb': 4,   # Strong per-acre value, expansion potential
    'Town':   3,   # Moderate value, community anchor potential
    'Rural':  3,   # Lower per-acre but volume/scale compensates
}
...
def score_rural_access(urbanization):
    """0-8 pts — rural/town institutions have better access to rural dev programs"""
    urb = (urbanization or '').split(':')[0].strip()
    if urb == 'Rural':   return 8
...
def compute_partnership_score(r):
    ...
    axis4 = s_re_market + s_acreage + s_land_type

    total = axis1 + axis2 + axis3 + axis4
    ...
    return {
        ...
        'pr_re_market_pts':            s_re_market,
        'pr_acreage_pts':              s_acreage,
        'pr_land_type_pts':            s_land_type,
        ...
        'pr_re_tier':                  STATE_RE_APPRECIATION.get(state, 2),
        'pr_acreage_used':             round(acreage, 1) if acreage else 0,
